...
from app_face_recognition.gauth import authenticate_google_drive
# cleanup_old_files
...
def main(images,group_image_object,root_path,group_img_path,grp_img_names_without_extention,thumb_dir):
    ...
    for imgPath in groupImagePaths:
        ...
        generate_thumbnails(drive,thumbFilesDir,face_locations,img,group_image_object,thumb_dir,grp_img_names_without_extention)
...
def generate_thumbnails(drive,thumbFilesDir,face_locations,img,group_image_object,thumb_dir,grp_img_names_without_extention):
    randomNumber=randint(1, 1000)
    for idx,(top, right, bottom, left) in enumerate(face_locations):
        file_name = str(randomNumber)+"_thumb_"+str(idx)
        print(file_name)
        roi = img[top-50:bottom+50,left:right]
        save_image(drive,thumbFilesDir,file_name,roi,group_image_object,grp_img_names_without_extention)
    return 
    
def save_image(drive,thumbFilesDir,file_name,img,group_image_object,group_image_name_without_extention):
    ...
    fileId=uploadToDrive(drive,file_name,out_file)
...
def uploadToDrive(drive,filename,out_file):
    file = drive.CreateFile({'parents': [{'id': '1T_4PMG_3a7-nfHfO4qzTT7N0ZYR_yhGz'}],'title': f'{filename}.jpg'})
    file.SetContentFile(out_file)
    file.Upload()
    return file['id']

def uploadToDriveTest(filename,out_file):
    gauth = GoogleAuth()
    gauth.CommandLineAuth()
    drive = GoogleDrive(gauth)
    file1 = drive.CreateFile({'parents': [{'id': '1T_4PMG_3a7-nfHfO4qzTT7N0ZYR_yhGz'}],'title': f'{filename}.txt'})  # Create GoogleDriveFile instance with title 'Hello.txt'.
